...
public class SSHKeyListener implements OnKeyListener{
	...
	public SSHKeyListener(
			SSHView.OnInputTextListener onInputTextListener,
						  VDUBuffer buffer,
						  String encoding) {
...
//			final boolean interpretAsHardKeyboard =
...
//							onInputTextListener.onInputText(String.valueOf('/'));
//						}
//						return true;
...
//						ourMetaState &= ~OUR_TRANSIENT;
//						if(onInputTextListener!=null){
//							onInputTextListener.onInputText(String.valueOf(0x09));
//						}
//						return true;
//					}
...
//				}
			...
			if (keyCode == KeyEvent.KEYCODE_UNKNOWN &&
				...
				if(onInputTextListener!=null){
					onInputTextListener.onInputText(new String(input));
				}
				return true;
			}
...
//					sendEscape();
				if (uchar < 0x80)
					if(onInputTextListener!=null){
						onInputTextListener.onInputText(new String(Character.toChars(uchar)));
					}
				else
					if(onInputTextListener!=null){
						onInputTextListener.onInputText(new String(Character.toChars(uchar)));
...
//							.getBytes(encoding));
			...
			case KeyEvent.KEYCODE_TAB:
				if(onInputTextListener!=null){
					onInputTextListener.onInputText(new String(Character.toChars(0x09)));
				}
				return true;
